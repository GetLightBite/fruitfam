...
def upload_video_from_bytes(video_bytes):
  c = boto.connect_s3('AKIAJHC3KOM63KGQA7TA', '85mJiGnhWFjvSXOa5cmLCOflHNr+Ze8XgaKa6+Dr')
  bucket_name = 'fruitfam'
  b = c.get_bucket(bucket_name, validate=False)
  random_video_name = ''.join(random.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for _ in range(50)) + '.mp4'
  k = Key(b)
  k.key = random_video_name
  k.set_contents_from_string(video_bytes)
  return 'https://s3.amazonaws.com/%s/%s' % (bucket_name, random_video_name)
